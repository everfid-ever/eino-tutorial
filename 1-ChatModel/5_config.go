...
/*
参数说明:
	Temperature (温度): 控制生成文本的随机性。值越高（如1.0以上），输出越随机和多样化；值越低（如0.0-0.3），输出越确定和集中。
	TopP (核采样): 通过概率质量来限制生成词汇的范围。值越低（如0.8），模型只考虑概率最高的词汇，输出更集中；值越高（如0.95-1.0），允许更多词汇，输出更丰富。
	MaxTokens (最大Token数): 限制生成响应的最大长度，防止过长输出。
	Stop (停止序列): 定义一组字符串，当生成文本包含这些字符串时，模型将停止生成。这有助于控制响应的结束点。
	PresencePenalty (存在惩罚): 控制模型引入新话题的倾向。正值增加引入新话题的可能
*/
